...
// 🔄 NEW: Fisher-Yates (Knuth) Shuffle Algorithm
...
// 🚀 NEW: Shuffle the questions array immediately after loading it
...
// NEW: Hint counter based on difficulty
let hintsRemaining;
...
// Get the hint button element (now the combined display/button)
const hintButtonEl = document.getElementById("hint-button");

// 🚩 NEW FLAG: To ensure the warning is shown only once
let timerWarningShown = false; 

// 🚩 NEW FUNCTION: To show the animated timer warning
function showTimerWarning() {
  if (timerWarningShown) return; // Exit if already shown
...
// Timer countdown 
timerDisplay.textContent = timeLeft; // show initial time immediately
const timer = setInterval(() => {
timeLeft--;
timerDisplay.textContent = timeLeft;

  // 🚩 LOGIC: Trigger warning when 10 seconds remain
  if (timeLeft === 10) {
    showTimerWarning();
  }
...
if (timeLeft <= 0) {
 clearInterval(timer);
  // Remove the warning if the quiz ends due to timeout
...
// NEW: Function to show the hint pop-up (no change, just cleanup for context)
function showHintPopup() {
  ...
  if (hintsRemaining > 0) {
    ...
    // Handle case where hints are 0 (including for hard difficulty's initial 0)
    modalTitle = 'Out of Hints';
    hintText = "Oops! You've run out of hints.";
    // For hard level (0 hints), the button still shows the pop-up
    if (selectedDifficulty === 'hard') {
      updateHintDisplay(); // Still update in case we need to re-disable later (though not needed here)
    }
  }
  
  // Helper function to show the modal (defined locally or globally)
...
// NEW: Function to update the hint button text and interactivity (no change, just cleanup for context)
function updateHintDisplay() {
  if (hintButtonEl) {
    // 1. Update the button text to show hints left (e.g., "Hint: 1")
    hintButtonEl.textContent = `Hint: ${hintsRemaining}`;
    
    // 2. Set the click handler
    hintButtonEl.onclick = showHintPopup;

    // 3. Disable the button only when hints are 0 AND the difficulty is NOT hard.
    // For hard (hintsRemaining=0 initially), we keep it enabled so the user can click it
    // to receive the "Out of Hints" message, which is still good feedback.
    if (selectedDifficulty !== 'hard') {
      hintButtonEl.disabled = (hintsRemaining === 0);
    } else {
      // For hard, it starts at 0, and remains clickable to show the 'Out of Hints' modal
...
// Show current question (updated to include hint display)
...
q.options.forEach(option => {
 ...
 // Fix for movie questions where answer is index 0, 1, 2, 3
 let optionValue = option;
  if (typeof q.answer === 'number') {
    // If the answer is an index, we need to pass the correct option text for comparison later
...
// Update the hint display on every question
updateHintDisplay(); 
}

// Handle answer click (existing code cleanup for mixed answer types)
function handleAnswer(selected) {
  const correctAnswer = questions[currentQuestion].answer;
  let isCorrect;

  if (typeof correctAnswer === 'number') {
    // Movie questions use number indices as answers in the data object
    isCorrect = selected === questions[currentQuestion].options[correctAnswer];
  } else {
    // All other questions use string answers
...
// End quiz (MODIFIED to remove warning if present)
function endQuiz() {
clearInterval(timer);
  
  // Remove the warning in case the quiz is completed before the timer runs out
  const warningEl = document.getElementById('timer-warning');
  if (warningEl) {
    warningEl.remove();
  }
